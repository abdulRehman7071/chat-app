...
const Chat = () => {
  ...
  // const Receiver = JSON.parse(localStorage.getItem("secondUser")) || [];
  const [senderName, setSenderName] = useState("");
  const sender = useSelector((store) => store.login.user);
  const Reciver = useSelector(
    (store) => store.login.reciver || { userName: "" }
  );
  const store = useSelector((store) => store.login.reciver || { userName: "" });
  ...
  return (
    <>
      <div className=" w-2/3 pl-4 p-4 min-h-screen flex flex-col justify-between ">
        <div className=" header flex justify-between border-b-[.1px] border-dashed pb-2  ">
